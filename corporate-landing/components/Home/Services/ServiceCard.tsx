...
const ServiceCard = ({ icon, image, title, category }: Props) => {
  return (
    <div className="rounded-lg group overflow-hidden bg-white">
      ...
      <div className="p-4">
        ...
        <h1 className="text-gray-700 mt-2 text-sm">{category}</h1>
        <p className="text-gray-800 mb-4 mt-2 font-semibold">{title}</p>
      </div>
    </div>
  );
};
